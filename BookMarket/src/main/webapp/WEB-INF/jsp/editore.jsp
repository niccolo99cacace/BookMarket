...
<section>
    ...
    <grid>
        ...
        <%-- stesso ragionamento di autore --%>
        <div col="2/3" txt="c" style="background-color:#E8E8E8">
            <a <c:if test="${pag > 1}">href="?id=${param.id}&pag=${pag - 1}&perpag=${perpag}"</c:if>>&larr;</a>
            &emsp;
            <c:forEach begin="1" end="${npag}" varStatus="loop">
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="?id=${param.id}&pag=${loop.index}&perpag=${perpag}">${loop.index}</a>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            &emsp;
